...
strategy("Vegas12", overlay=true,pyramiding = 20)
//-------------------------------------------
//-------------------------------------------
// Inputs
...
//波段多
if (inDateRange and crossunder(f4,f2))// 
    strategy.entry("buy", strategy.long,1, when=macd>0, comment = "买L")
buyclose=crossunder(f0,f1) 
strategy.close("buy", when = buyclose, comment = "关L")
//多策略1
if (inDateRange and crossover(close , f3) and macd>0 and f3>f6)
    strategy.entry("buy1", strategy.long,1, comment = "买M")
buyclose1=crossunder(close,upper) 
if (macd<0 or f3<f6)
...
//多策略3
if (inDateRange and  (macd>0) and crossunder(close,f1) and f1>f4) // 
    strategy.entry("buy3", strategy.long,1, comment = "买S")
buyclose3=crossunder(close,upper)
if (macd<0 or f1<f4)
